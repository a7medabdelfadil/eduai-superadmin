...
const EditAdmin = () => {
  ...
  const [countryCode, setCountryCode] = useState("MA"); // default for Morocco
  ...
  const { data: adminDetails, isSuccess: adminDetailsSuccess } =
    useGetAdminQuery({ token, id: params.id });
  const { data: dataEmployeeStatus, isSuccess: successEmployeeStatus } =
    useEmployeeStatusQuery(token);
  const [
    updateAdmin,
    { data, isSuccess, isLoading, isError, error, originalArgs },
  ] = useUpdateAdminMutation();
  ...
  const handleSend = async (e: any) => {
    ...
    if (!name_en) newErrors.name_en = "English name is required.";
    if (!name_ar) newErrors.name_ar = "Arabic name is required.";
    if (!name_fr) newErrors.name_fr = "French name is required.";
    if (!about) newErrors.about = "About field is required.";
    if (!employeeStatus) newErrors.employeeStatus = "Status is required.";
                  ...
                  >
                    {successEmployeeStatus && (
                      <>
                        ...
                        {Object.keys(dataEmployeeStatus.data).map(
                          (key: any) => (
                            <option key={key} value={key}>
                              {dataEmployeeStatus.data[key]}
                            </option>
                          )
                        )}
                      </>
